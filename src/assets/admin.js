...
Grid.prototype.getData = function () {
    ...
    var data = [];
    var counter = 0;
    var searchTerm = this.search.toLowerCase().latinise();
    for (var row = 0; row < this.data.length; row++) {
        for (var col = 0; col < this.data[row].length; col++) {
            var searchIn = String(this.data[row][col]).toLowerCase().latinise();
            if ($.inArray(col, this.searchIndexes) > -1 && searchIn.indexOf(searchTerm) > -1) {
